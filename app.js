let cardList = `1457 First printed music volume
...
2009 Avatar
2009 Bad Romance
2009 Relapse`.split('\n') // new array of card strings

function cardifyList (cardList) {
  //cardList expects array of card strings
  return cardList.map(function(card){
    card = card.split(' ')
    return {
      year: card[0], // first element as string
      title: card.slice(1).join(' ') // every other element as string
    }
  })
}

cardList = cardifyList(cardList)
